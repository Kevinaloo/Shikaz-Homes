...
async function loadReferrals() {
  try {
    const { data } = await supabase.from("kv_store").select("value").eq("key","shikaz:referrals").single();
...
async function loadCommissionSettings() {
  try {
    const { data } = await supabase.from("kv_store").select("value").eq("key","shikaz:commission").single();
...
function ReferAndEarnSection({ onOpenDashboard }) {
  ...
  const share = () => {
    if(navigator.share) navigator.share({ title:"Shikaz Homes", text:"Book a premium Nairobi stay with this exclusive link!", url:code });
    else copy();
  };

  return (
    <section style={{background:"linear-gradient(160deg,#0E2B1F 0%,#1a3d2b 50%,#0E2B1F 100%)",padding:"5rem 1.5rem",position:"relative",overflow:"hidden"}}>
      {/* Background pattern */}
      <div style={{position:"absolute",inset:0,backgroundImage:"radial-gradient(circle at 2px 2px,rgba(197,151,58,0.07) 1px,transparent 0)",backgroundSize:"28px 28px",pointerEvents:"none"}}/>
      {/* Glowing orbs */}
      <div style={{position:"absolute",top:"-60px",right:"-60px",width:"300px",height:"300px",borderRadius:"50%",background:"radial-gradient(circle,rgba(197,151,58,0.12),transparent 70%)",pointerEvents:"none"}}/>
      <div style={{position:"absolute",bottom:"-40px",left:"-40px",width:"200px",height:"200px",borderRadius:"50%",background:"radial-gradient(circle,rgba(76,175,125,0.1),transparent 70%)",pointerEvents:"none"}}/>
      ...
      <div style={{maxWidth:"900px",margin:"0 auto",position:"relative"}}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(min(100%,360px),1fr))",gap:"3rem",alignItems:"start"}}>
          ...
          {/* Left — pitch */}
          <div>
            <div style={{display:"inline-flex",alignItems:"center",gap:"0.5rem",padding:"0.3rem 0.9rem",background:"rgba(197,151,58,0.15)",border:"1px solid rgba(197,151,58,0.3)",borderRadius:"20px",marginBottom:"1.2rem"}}>
              <span style={{fontSize:"0.9rem"}}>💰</span>
              <span style={{fontSize:"0.65rem",fontWeight:700,letterSpacing:"0.2em",textTransform:"uppercase",color:C.gold}}>Referral Programme</span>
            </div>
            <h2 style={{fontFamily:"'Playfair Display',serif",fontSize:"clamp(2rem,4vw,3rem)",color:"#F7F2EA",fontWeight:400,lineHeight:1.15,marginBottom:"1rem"}}>
              Refer & <em style={{color:C.gold,fontStyle:"italic"}}>Earn</em>
            </h2>
            <p style={{fontSize:"0.92rem",color:"rgba(247,242,234,0.65)",lineHeight:1.85,marginBottom:"1.5rem"}}>
              Share Shikaz Homes with your network. Every time someone books using your unique link and pays, you earn a commission — sent directly to your M-Pesa.
            </p>
            {[
              {icon:"🔗",t:"Get your link",d:"Enter your name and phone to generate your unique referral link."},
              {icon:"📲",t:"Share it",d:"Send to friends, post on WhatsApp, Instagram, or anywhere online."},
              {icon:"💳",t:"Earn commission",d:"When they book and pay, your commission is automatically tracked."},
              {icon:"📱",t:"Claim to M-Pesa",d:"Check your balance anytime and request a payout to your phone."},
            ].map((s,i)=>(
              <div key={i} style={{display:"flex",gap:"1rem",marginBottom:"1rem",alignItems:"flex-start"}}>
                <div style={{width:"32px",height:"32px",borderRadius:"50%",background:"rgba(197,151,58,0.15)",border:"1px solid rgba(197,151,58,0.3)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1rem",flexShrink:0}}>{s.icon}</div>
                <div>
                  <div style={{fontSize:"0.82rem",fontWeight:600,color:"#F7F2EA",marginBottom:"0.1rem"}}>{s.t}</div>
                  <div style={{fontSize:"0.75rem",color:"rgba(247,242,234,0.5)",lineHeight:1.5}}>{s.d}</div>
                </div>
              </div>
            ))}
            <button onClick={onOpenDashboard}
              style={{marginTop:"0.5rem",background:"transparent",border:"1px solid rgba(197,151,58,0.4)",color:C.gold,padding:"0.65rem 1.4rem",borderRadius:"4px",fontSize:"0.75rem",fontWeight:600,letterSpacing:"0.15em",textTransform:"uppercase",cursor:"pointer",transition:"all 0.2s"}}
          ...
          <div style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(197,151,58,0.2)",borderRadius:"14px",padding:"2rem",backdropFilter:"blur(10px)"}}>
            <div style={{fontSize:"0.7rem",letterSpacing:"0.25em",textTransform:"uppercase",color:C.gold,marginBottom:"1.2rem",fontWeight:600}}>Get Your Referral Link</div>
              ...
              <>
                {[
                  ...
                  <div key={f.label} style={{marginBottom:"0.9rem"}}>
                    <label style={{display:"block",fontSize:"0.62rem",letterSpacing:"0.15em",textTransform:"uppercase",color:"rgba(247,242,234,0.5)",marginBottom:"0.4rem"}}>{f.label}</label>
                    <input type={f.type} value={f.val} onChange={e=>{f.set(e.target.value);setErr("");}}
                      placeholder={f.ph}
                      style={{width:"100%",padding:"0.8rem 1rem",background:"rgba(255,255,255,0.06)",border:"1px solid rgba(197,151,58,0.25)",borderRadius:"6px",color:"#F7F2EA",fontSize:"0.9rem",outline:"none",transition:"border-color 0.2s",}}
                      onFocus={e=>e.target.style.borderColor=C.gold}
                      onBlur={e=>e.target.style.borderColor="rgba(197,151,58,0.25)"}/>
                  </div>
                ))}
                {err&&<div style={{fontSize:"0.75rem",color:C.error,marginBottom:"0.7rem"}}>{err}</div>}
                <button onClick={generate} disabled={loading}
                  style={{width:"100%",padding:"1rem",background:`linear-gradient(135deg,${C.gold},${C.goldLight})`,color:C.obsidian,border:"none",borderRadius:"8px",fontSize:"0.85rem",fontWeight:700,letterSpacing:"0.15em",textTransform:"uppercase",cursor:loading?"not-allowed":"pointer",transition:"all 0.2s",boxShadow:`0 8px 24px rgba(197,151,58,0.3)`}}>
                  {loading ? "Generating…" : "✨ Generate My Link"}
                </button>
                <p style={{textAlign:"center",fontSize:"0.68rem",color:"rgba(247,242,234,0.35)",marginTop:"0.8rem",lineHeight:1.5}}>Your phone number is used only to identify your account and send payouts.</p>
              </>
            ) : (
              <div style={{animation:"fadeIn 0.4s ease"}}>
                <div style={{textAlign:"center",marginBottom:"1.5rem"}}>
                  <div style={{fontSize:"2.5rem",marginBottom:"0.5rem"}}>🎉</div>
                  <div style={{fontFamily:"'Playfair Display',serif",fontSize:"1.1rem",color:"#F7F2EA",marginBottom:"0.2rem"}}>Your link is ready!</div>
                  <div style={{fontSize:"0.72rem",color:"rgba(247,242,234,0.5)"}}>Share it anywhere — you earn every time someone books</div>
                </div>
                {/* Link display */}
                <div style={{background:"rgba(0,0,0,0.3)",border:"1px solid rgba(197,151,58,0.3)",borderRadius:"8px",padding:"0.9rem 1rem",marginBottom:"1rem",wordBreak:"break-all",fontSize:"0.78rem",color:C.gold,fontFamily:"monospace"}}>{code}</div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0.6rem",marginBottom:"1rem"}}>
                  <button onClick={copy}
                    style={{padding:"0.8rem",background:copied?"rgba(76,175,125,0.2)":"rgba(197,151,58,0.1)",border:`1px solid ${copied?C.success:"rgba(197,151,58,0.3)"}`,borderRadius:"6px",color:copied?C.success:C.gold,fontSize:"0.78rem",fontWeight:600,cursor:"pointer",transition:"all 0.2s"}}>
                    {copied?"✓ Copied!":"📋 Copy Link"}
                  </button>
                  <button onClick={share}
                    style={{padding:"0.8rem",background:"rgba(37,211,102,0.1)",border:"1px solid rgba(37,211,102,0.3)",borderRadius:"6px",color:"#25D366",fontSize:"0.78rem",fontWeight:600,cursor:"pointer",transition:"all 0.2s"}}>
                    📲 Share
                  </button>
                </div>
                <button onClick={()=>setCode("")}
                  style={{width:"100%",padding:"0.65rem",background:"transparent",border:"1px solid rgba(247,242,234,0.15)",borderRadius:"6px",color:"rgba(247,242,234,0.4)",fontSize:"0.72rem",cursor:"pointer"}}>
                  Generate for different number
                </button>
              </div>
            )}
          </div>
...
// ── Admin Shell ───────────────────────────────────────────────────
function AdminDashboard({ listings, bookings, onNavigate, onListingUpdate, onListingCreate, onListingDelete, promoConfig, onPromoSave, siteContent, onSiteContentSave, referrals, commissionSettings, onCommissionSave, onReferralsUpdate }) {
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    Promise.all([loadListings(),loadBookings(),loadPromos(),loadSiteContent(),loadReferrals(),loadCommissionSettings()]).then(([ls,bs,pc,sc,refs,cs])=>{
      ...
      if(urlRef){ setPendingRefCode(urlRef); sessionStorage.setItem("shikaz_ref",urlRef); }
      else {
        const stored = sessionStorage.getItem("shikaz_ref");
  ...
  const handleBookingMade=async(booking)=>{
    ...
    const refCode = pendingRefCode || sessionStorage.getItem("shikaz_ref");
